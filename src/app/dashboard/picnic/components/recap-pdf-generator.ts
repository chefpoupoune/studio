...
const isMobile = useMobile();
   ...
   useEffect(() => {
    ...
    if (overrideTemplateId && overrideTemplateId !== 'auto') {
        ...
        if (allMonthlyTemplates[monthIndex]?.[templateIndex]) {
            ...
            console.log(`[Recap EFFECT ActiveMenuTemplate] Override active for template: ${template.name || 'Untitled'}`);
            return; 
        } else {
             console.log(`[Recap EFFECT ActiveMenuTemplate] Override ID ${overrideTemplateId} not found, falling back to auto.`);
             setOverrideTemplateId('auto'); // Reset to auto if invalid
        }
    }

    // Original "auto" logic
    console.log("[Recap EFFECT ActiveMenuTemplate] Calculating auto. initialDataLoaded:", initialDataLoaded, "allMonthlyTemplates keys:", Object.keys(allMonthlyTemplates).length);
    ...
    if (templateIndexForMonth !== null && templateIndexForMonth !== undefined && allMonthlyTemplates[currentMonthIndex]?.[templateIndexForMonth]) {
        ...
        setActiveMenuTemplateForRecap(template);
        console.log("[Recap EFFECT ActiveMenuTemplate] Auto-selected template set:", template.weeklyNote || "No note");
    } else {
        setActiveMenuTemplateForRecap(null);
        console.log("[Recap EFFECT ActiveMenuTemplate] No active template found or set for this month/week (auto).");
    }
 ...
 const generateRecapPdf = async () => {
    ...
    try {
        ...
        // Correction ici: 'format' a été renommé en 'pageFormat' pour éviter le conflit
        ...
        doc.setFontSize(pdfSettings.defaultFontSize);
        doc.text("Récapitulatif Journalier des Pains Nécessaires", pdfSettings.marginLeft, currentY);
        currentY += pdfSettings.defaultFontSize * 0.7 + 4;
   ...
   if (isMobile) {
    return (
      <div className="space-y-6 p-1 md:p-4">
                            ...
                            <div className="space-y-2">
                                <div>
                                    ...
                                    <div className="grid grid-cols-5 gap-1 text-center text-xs">
                                        ...
                                        {DAYS_OF_WEEK_KEYS.map(day => <div key={day}>{recap.baguetteCounts[day] > 0 ? recap.baguetteCounts[day] : '-'}</div>)}
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="grid grid-cols-5 gap-1 text-center text-xs">
                                        {DAYS_OF_WEEK_KEYS.map(day => <div key={day} className="font-semibold capitalize">{DAY_LABELS[day].substring(0,3)}</div>)}
                                        {DAYS_OF_WEEK_KEYS.map(day => <div key={day}>{recap.falucheCounts[day] > 0 ? recap.falucheCounts[day] : '-'}</div>)}
                                    </div>
                                </div>
                                {recap.observation && (
        ...
        <Card>
            <CardHeader>
                <CardTitle>Récapitulatif Pains</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2 text-sm">
                    {[{label: 'Pain (Total)', type: 'pain'}, {label: 'Baguette', type: 'baguette'}, {label: 'Faluche', type: 'faluche'}].map(item => (
