 ...
 const handleTempCellClick = (dateStr: string, equipmentId: string, tempValueToMark: number | undefined) => {
  ...
  const generatePdfForEquipment = () => {
    ...
    try {
      ...
      const head: any[] = [
        [
          { content: 'Zone', rowSpan: 2, styles: { valign: 'middle', halign: 'center', fontStyle: 'bold', minCellWidth: 18, cellWidth: COL_WIDTH_ZONE_PX, fontSize: FONT_SIZE_ZONE_LABEL_PX } },
          { content: 'T°C', rowSpan: 2, styles: { valign: 'middle', halign: 'center', fontStyle: 'bold', minCellWidth: 8, cellWidth: COL_WIDTH_TEMP_VALUE_PX, fontSize: FONT_SIZE_TEMP_VALUES_PX } },
          ...monthData.map(day => ({ content: `${day.dayOfMonth}\n${day.dayName.substring(0,1)}`, styles: { halign: 'center', fontSize: FONT_SIZE_DAY_HEADER_PX, cellWidth: COL_WIDTH_DAY_PX, minCellHeight: 6 } })) 
        ]
      ];
          ...
          if (indexInZoneValues === 0) { 
             row.push({ 
                ...
                styles: { 
                    ...
                    halign: 'center', 
                    fontStyle: 'bold', 
                    fontSize: FONT_SIZE_ZONE_LABEL_PX,
                    fillColor: zone.pdfColor,
                    cellWidth: COL_WIDTH_ZONE_PX,
                } 
            });
          }
          row.push({ content: `${tempVal}°C`, styles: { halign: 'center', fontSize: FONT_SIZE_TEMP_VALUES_PX, cellPadding: 0.2, cellWidth: COL_WIDTH_TEMP_VALUE_PX } });
      ...
      const timeRow: any[] = [{content: 'Heure', colSpan: 2, styles: {fontStyle: 'bold', fontSize: FONT_SIZE_INPUT_FIELDS_PX, halign: 'right', cellPadding: 0.2, cellWidth: COL_WIDTH_ZONE_PX + COL_WIDTH_TEMP_VALUE_PX}}];
      monthData.forEach(day => {
        const record = getRecord(day.date, selectedEquipmentData.id);
        timeRow.push({content: record.time || '-', styles: {halign: 'center', fontSize: FONT_SIZE_INPUT_FIELDS_PX, cellPadding: 0.2, cellWidth: COL_WIDTH_DAY_PX, fillColor: day.isWeekend ? [230,230,230] : undefined}});
      });
      body.push(timeRow);

      const operatorRow: any[] = [{content: 'Opérateur', colSpan: 2, styles: {fontStyle: 'bold', fontSize: FONT_SIZE_INPUT_FIELDS_PX, halign: 'right', cellPadding: 0.2, cellWidth: COL_WIDTH_ZONE_PX + COL_WIDTH_TEMP_VALUE_PX}}];
      monthData.forEach(day => {
        const record = getRecord(day.date, selectedEquipmentData.id);
        operatorRow.push({content: record.operator || '-', styles: {halign: 'center', fontSize: FONT_SIZE_INPUT_FIELDS_PX, cellPadding: 0.2, cellWidth: COL_WIDTH_DAY_PX, fillColor: day.isWeekend ? [230,230,230] : undefined}});
      });
      ...
      doc.autoTable({
        ...
        body: body,
        theme: 'grid',
        headStyles: { halign: 'center', fontSize: FONT_SIZE_DAY_HEADER_PX, cellPadding: 0.5, fillColor: primaryColorForPdf, textColor: [0,0,0] },
        styles: { fontSize: FONT_SIZE_TEMP_VALUES_PX, cellPadding: 0.2, minCellHeight: ROW_HEIGHT_TEMP_PX }, 
        didDrawPage: (data) => {
              ...
              <div className="overflow-x-auto border rounded-md">
                <Table className="min-w-full table-fixed text-[6px] leading-none">
                  <TableHeader className="sticky top-0 z-30 bg-card shadow-sm">
                    <TableRow className={cn("h-auto p-0", `h-[${ROW_HEIGHT_INPUT_PX * 1.5}px]`)}>
                      <TableHead className={cn("sticky left-0 z-20 bg-card border-r text-center p-0 align-middle leading-tight", `w-[${COL_WIDTH_ZONE_PX}px]`)} style={{fontSize: `${FONT_SIZE_ZONE_LABEL_PX}px`}}>Zone</TableHead>
                      <TableHead className={cn("sticky left-[var(--col-zone-width)] z-20 bg-card border-r text-center p-0 align-middle leading-tight", `w-[${COL_WIDTH_TEMP_VALUE_PX}px]`)} style={{fontSize: `${FONT_SIZE_TEMP_VALUES_PX}px`}}>t°</TableHead>
                      {monthData.map(day => (<TableHead key={day.date} className={cn("text-center p-0 h-auto leading-tight", `w-[${COL_WIDTH_DAY_PX}px]`, day.isWeekend && "bg-muted/30")} style={{fontSize: `${FONT_SIZE_DAY_HEADER_PX}px`}}>{day.dayOfMonth}<br/>{day.dayName.substring(0,1)}</TableHead>))}
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {dynamicTempZones.map(zone => 
                       zone.values.slice().reverse().map((tempValue, indexInZone) => ( 
                         <TableRow key={`${zone.type}-${tempValue}`} className={cn("h-auto p-0", `h-[${ROW_HEIGHT_TEMP_PX}px]`)}>
                            {indexInZone === 0 ? (
                                <TableCell
                                  rowSpan={zone.values.length}
                                  className={cn(zone.color, zone.textColor, "font-semibold align-middle text-center p-0 sticky left-0 z-10 border-r leading-none", `w-[${COL_WIDTH_ZONE_PX}px]`)}
                                  style={{padding: '0px'}}
                                >
                            ...
                            <TableCell className={cn("font-mono text-center border-r sticky left-[var(--col-zone-width)] z-10 bg-card p-0 py-0", `w-[${COL_WIDTH_TEMP_VALUE_PX}px]`)} style={{fontSize:`${FONT_SIZE_TEMP_VALUES_PX}px`, padding:'0px'}}>
                                {tempValue}°
                            </TableCell>
                            {monthData.map((day) => {
                                const record = getRecord(day.date, selectedEquipmentData.id);
                    ...
                    <TableRow className={cn("bg-card/90 sticky bottom-[var(--row-input-height)] z-20 h-auto", `h-[${ROW_HEIGHT_INPUT_PX}px]`)}>
                        ...
                        {monthData.map(day => (
                            <TableCell key={`time-${day.date}`} className={cn("p-0 border-t h-full", `w-[${COL_WIDTH_DAY_PX}px]`, day.isWeekend && "bg-muted/25")}>
                                <Input type="text" placeholder="HH:mm" defaultValue={getRecord(day.date, selectedEquipmentData.id).time} 
                                       ...
                                       className={cn("h-full text-center p-0 border-0 rounded-none focus-visible:ring-1 focus-visible:ring-ring bg-transparent w-full leading-tight")} pattern="([01]?[0-9]|2[0-3]):[0-5][0-9]" style={{fontSize: `${FONT_SIZE_INPUT_FIELDS_PX}px`, padding: '0px'}} disabled={day.isWeekend || isSaving}/>
                            </TableCell>
                        ))}
                    </TableRow>
                    <TableRow className={cn("bg-card/90 sticky bottom-0 z-20 h-auto", `h-[${ROW_HEIGHT_INPUT_PX}px]`)}>
                        ...
                        {monthData.map(day => (
                            <TableCell key={`op-${day.date}`} className={cn("p-0 border-t h-full", `w-[${COL_WIDTH_DAY_PX}px]`, day.isWeekend && "bg-muted/25")}>
                                <Input type="text" placeholder="Op." defaultValue={getRecord(day.date, selectedEquipmentData.id).operator} 
                                       ...
                                       className={cn("h-full text-center p-0 border-0 rounded-none focus-visible:ring-1 focus-visible:ring-ring bg-transparent w-full leading-tight")} maxLength={10} style={{fontSize: `${FONT_SIZE_INPUT_FIELDS_PX}px`, padding: '0px'}} disabled={day.isWeekend || isSaving}/>
                            </TableCell>
                        ))}
                    </TableRow>
                  </TableBody>
